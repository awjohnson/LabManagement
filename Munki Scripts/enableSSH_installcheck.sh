...
if [[ $(dscl /Local/Default read Groups/com.apple.access_ssh NestedGroups | grep "$admin_uuid" | wc -l) -eq 0 ]]; then
    echo "EnableSSH: admin group not nested in $ssh_group!"
    exit 0
fi

echo "EnableSSH: everything seems as it should be, no install needed."
exit 1
